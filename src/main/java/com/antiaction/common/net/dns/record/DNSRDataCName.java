 ...
 * 21-Nov-2001 : buildPacket() RDLen +2 offset error.
 *
 */
 ...
 */
public class DNSRDataCName implements DNSRDataInterface {
	...
	protected static int rtype = DNSType.CNAME;
	/** Canonical Name. */
	...
	public DNSRDataCName() {
 	 ...
 	 */
 	public Object clone() {
 		DNSRDataCName obj = new DNSRDataCName();
		obj.name = name;
 		return obj;
 	}
	...
	public byte[] buildPacket(DNSName dnsname, int globalIdx) throws DNSNameException {
		...
		if ( debug ) {
			System.out.println("-> DNSRDataCName.buildPacket()");
	...
	public void disassemblePacket(DNSName dnsname, byte[] pDat, int pIdx, int pLen) throws DNSNameException, DNSRDataException {
		...
		if ( debug ) {
			System.out.println("-> DNSRDataCName.disassemblePacket() - idx=" + pIdx);
		}
	...
	// Canonical Name
		...
		if ( debug ) {
			System.out.println("    cname: " + name);
			System.out.println("<- DNSRDataCName.disassemblePacket() - Len=" + disLen);
		}
	}
