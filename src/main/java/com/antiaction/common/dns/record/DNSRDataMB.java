 ...
 */
public class DNSRDataMB implements DNSRDataInterface {
	...
	/** Record of type MB. */
	protected static int rtype = DNSType.MB;
	/** Mailbox Domain Name. */
	...
	public byte[] buildPacket(DNSName dnsname, int globalIdx) throws DNSNameException {
		...
		if ( debug ) {
			System.out.println("-> DNSRDataMB.buildPacket()");
	...
	public void disassemblePacket(DNSName dnsname, byte[] pDat, int pIdx, int pLen) throws DNSNameException, DNSRDataException {
		...
		if ( debug ) {
			System.out.println("-> DNSRDataMB.disassemblePacket() - idx=" + pIdx);
		}
	...
	// Mailbox Domain Name
		...
		if ( debug ) {
			...
			System.out.println("<- DNSRDataMB.disassemblePacket() - Len=" + disLen);
		}
	}
	 ...
	 */
	public void setMailBox(String name) {
		this.name = name;
	}
